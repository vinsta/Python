...
def ValidateSCF(scfpath):
	for filename in os.listdir(scfpath):
		...
		try:
			...
			root = etscf.getroot()
			logfile = open("{}.log".format(filename), "w", encoding = "utf8")
			WriteLog(logfile, "{}\n".format("#" * 50))
			WriteLog(logfile, "{} validattion begin\n".format(filename))
			print("{} validattion begin".format(filename))
		...
		for data in cmdata:
			...
			for mo in MOs:
				...
				if distname.find("/FTM-") != -1 or classname.find("_R") != -1:
					continue
				result = ValidateInstanceId(logfile, classname, distname)
				if result is True:
					for param in mo:
						ValidateParamValue(logfile, classname, distname, param.attrib["name"], param)

				ValidateMandatoryParams(logfile, classname, distname, mo)
		ValidateMandatoryClass(logfile, MOs)
		WriteLog(logfile, "{} validation end\n".format(filename))
		print("{} validattion end".format(filename))
		logfile.close()

def ValidateMandatoryParams(logfile, clsname, distname, mo):
	try:
			...
			if paramdict[key].mandatory is True:
				...
				for param in mo:
					if param.attrib["name"] == paramname:
						...
						break
				if found is False:
					WriteLog(logfile, "Mandatory parameter {}-{} is missing!\n".format(distname, paramname))
...
def ValidateMandatoryClass(logfile, objlist):
	for key in modict:
		if key.find("/FTM") != -1:
			continue
		name = ""
...
def ValidateInstanceId(logfile, clsname, distname):
	result = True
	instid = int(distname[distname.rindex("-") + 1 :])
	key = "{}-{}".format(clsname, "instanceid")
	try:
		if not paramdict[key].min <= instid <= paramdict[key].max:
			WriteLog(logfile, "{} instance id {} not in range [{}-{}]!\n".format(distname, instid, paramdict[key].min, paramdict[key].max))
			result = False
	except KeyError:
		WriteLog(logfile, "Unknown object {}!\n".format(distname))
		result = False
	return result

def ValidateParamValue(logfile, classname, distname, name, param, islist = False):
	paramname = param.attrib["name"]
